...
#
#
#when player position = food position -> eat
...
def initialize_game(player, game, food, agent):
    #11 possible states, first one is food right / move left
...
def plot_seaborn(array_counter, array_score):
    ...
    ax = sns.regplot(np.array([array_counter])[0], np.array([array_score])[0],
...
def run():
    ...
    record = 0
     ...
     # learning loops # 
    # # # # # # # # # 
    while counter_games < trains:
        # init game with size 440*440
        game = Game(440, 440)
        #init player
        player1 = game.player
        #new Food object
        ...
        while not game.crash:
            ...
            # perform random actions based on agent.epsilon,
            # or choose the action
            if randint(0, 200) < agent.epsilon:
                final_move = to_categorical(randint(0, 2), num_classes=3)
            else:
                # predict action based on the old state
                    #agent.model() gets numpy array and returns numpy array
